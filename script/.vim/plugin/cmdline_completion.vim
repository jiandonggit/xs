...
if !hasmapto('<Plug>CmdlineCompletionBackward','c')
    ...
    cmap <expr> <c-p> getcmdpos()==1? '<up><home>' ;
                \ '<plug>CmdlineCompletionBackward'
endif

if !hasmapto('<Plug>CmdlineCompletionForward','c')
    " cmap <unique> <silent> <C-P> <Plug>CmdlineCompletionForward
    cmap <expr> <c-p> getcmdpos()==1? '<down><home>' ;
                \ '<plug>CmdlineCompletionForward'
endif
...
function! s:CmdlineCompletion(backward)
    ...
    if ( b:cc_word_index < 0 || b:cc_word_index >= len(b:cc_word_list))
        ...
        while b:cc_buffer_index <= bufnr('$')
            ...
            if b:cc_search_status_current
                ...
                let b:cc_search_status_current = s:SearchCurrent(a:backward)
                ...
                let b:cc_search_status = 
                            \ s:SearchBuffer(a:backward, b:cc_buffer_index)
